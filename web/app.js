...
//
// STATIC, and it must stay static: `await import()` here would defer the
// `defineComponent` calls past the `enableRouting` on the last line, so the router
// would resolve a route to a component the registry does not have yet.
//
// This is no longer a CYCLE. `shared.js` used to import this module's helpers back,
// which worked only because it touched them inside method bodies rather than at
// evaluation time — nothing enforced that, and getting it wrong renders a blank page
...
/**
 * `ApiMap` lives in `core.js` beside the `api()` that is generic over it. A JSDoc
 * typedef is FILE-SCOPED, so it has to be re-imported here by name — without this the
 * 30-odd `@type {ApiMap[...]}` annotations below silently degrade to `any` rather than
 * erroring, on a `checkJs` project where that is the whole point of the map.
 * @typedef {import('./core.js').ApiMap} ApiMap
 */
const dashUrl = (u) => `/u/${u}/`;
const treeUrl = (u, prefix) => `/u/${u}/tree/` + (prefix ? prefix + '/' : '');
const goTree = (u, prefix) => go(treeUrl(u, prefix));
  ...
  reqRow(rq) {
    ...
    return html`<div class="dreq ${rq.auditMoved || (rq.assertionsMoved || []).length ? 'moved' : ''}">
      <div class="dbugh">
        ...
        ${when((rq.pointersFired || []).length, () => html`<span class="bchip poss" title="something this rule watches moved in this diff — a prior on where to look, never a verdict">pointer fired</span>`)}
      ...
      ${each(rq.pointersFired || [], pt => html`<div class="dstep"><span class="stn">▸ ${pt.via}</span> <span class="dim">${pt.rank} · ${pt.rationale}</span></div>`, pt => pt.id)}
