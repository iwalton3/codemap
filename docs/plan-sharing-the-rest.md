...
### Only HUMAN edges travel

The rule already exists for docs and applies unchanged: analyzer output is regenerated by
every machine from the code, so publishing it shares a copy that can never be refreshed
or judged stale. `edges.generated_by` is the discriminator and is already there.
...
### Merge: wall-clock resolves, divergence is queued, an agent repairs

**Owner's call, and the architecture is optimistic-merge-detect-repair rather than
get-the-merge-right.** Newest wall-clock `at` wins, tie-broken by event id — the
tie-break is not optional, because `at` alone is not a total order and two clones sharing
a timestamp would otherwise pick differently, which breaks CONVERGENCE.

**Silent last-write-wins is not enough on its own, and this is the one thing to get
right.** If the loser simply vanishes there is no conflict to queue, and the whole
repair model has nothing to work on. So the fold does both, exactly as shared triage
already does for stakes: it resolves to an answer so nothing is blocked, AND records the
divergence so a person or an agent can act. Concurrent publications of one node's wiring
by two writers, neither having seen the other, are a divergence — a later publication
that SAW the other is a decision and is not.
...
One finding survives the change and must be answered here instead: **an edge cites node
ids, and a node id is not derivation-dependent** (it is a doc id, minted by whoever wrote
the doc), so edges do not inherit the anchor-provenance problem. What they do inherit is
ORDER of arrival — an edge can arrive before the node it cites. The fold must keep such
an edge rather than dropping it, and the surface must render the node as pending rather
than absent, or a slow-arriving doc silently loses its wiring.
