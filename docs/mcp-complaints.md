...
# Workflow issues — friction while USING the review end-to-end
...
> that it was addressed* is real, and the cut that separates them is the number, not the prose.
> `remediation` is ALSO rendered on the shared view now, which was this item's cheaper fix and
> is worth having regardless — it was on every record and no surface.
...
> **OPEN.** A unique-prefix resolution or a "did you mean" is still worth doing.
...
> **PARTLY MOOT, and clarified.** The confirmed-finding gate it failed to lift no longer exists
> for `comment`. Its description now says it lifts only the already-posted refusal.
...
> **OPEN**, and the sharpest of the remaining three — it turns "what is still open" into a
> three-step detour on a normal-sized pull request.
...
The complaint logged earlier in `docs/mcp-complaints.md` — *"`request_human` has no
...
`docs/mcp-complaints.md` holds the per-tool-call issues from the same period,
