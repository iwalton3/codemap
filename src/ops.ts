...
export async function backlogOn(root: string, input: { id: string; until: string; reason: string; ref?: { system: string; key?: string; url?: string } }) {
  ...
  if (!w.finding.shared) {
    const wit = await shared.backlogWitnessFor(root, input.id);
    if (wit.unfetched) return { error: shared.unfetchedCarryError(input.id) };
    return backlogLocalFinding(root, input.id, { until, reason, ref: input.ref, witness: wit.witness });
  }
...
export async function deferFinding(
  ...
  // The bulk warning lives in `acceptFinding`, which is where every route converges —
  // this one, the MCP tool over it, and the web button, which posts straight there. It
  // used to be here, on one caller, so the surface a person actually sweeps a queue from
  // was the one surface it could not reach.
