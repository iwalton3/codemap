...
const tools: Tool[] = [
  ...
  },
  {
    ...
    inputSchema: obj({
      ...
      backlog: { type: "boolean", description: "The deferral register instead of the working queue: bugs somebody deliberately put off, with the deadline that brings each back." },
  ...
  },
  {
    ...
    inputSchema: obj({
      ...
      fromSection: { type: "string", description: "`move_section`: the section path to move, WITH its subtree — moving \"Credit\" takes \"Credit/Limits\" with it. Sections are derived from the rules filed in them, so this must be a path something files under." },
      toSection: { type: "string", description: "`move_section`: where it lands. Renaming and re-parenting are one act on a path tree. A destination an untouched rule already occupies is refused: that merges two headings, and the index is how a reader finds the rule governing an area. Fixing a case split (\"credit\" -> \"Credit\") IS a legal move." },
  ...
  {
    name: "revise_spec",
    description: "Correct a DRAFT proposal's title or narrative — your own mistake, fixed where the ratifier reads it. Open to any actor: correcting a proposal is authoring it, and the asymmetry is in adoption, not authorship.\n\nRefused once the spec is ratified. Immutability attaches when a claim becomes binding — a draft binds nothing, a ratified spec is the act that produced a rule — so after adoption the path is a new spec amending it.\n\nIt MUTATES the text rather than appending a correction, because the whole trade this surface makes is that a principal reads one current text instead of 5,000 lines; a correction chain they have to reassemble is that trade failing at its last step. Nothing is lost — the prior wording is kept underneath in `revisions`. Do NOT put a correction in a `comment`: the narrative is what renders, so the ratifier reads the wrong framing first and the retraction second.",
    inputSchema: obj({
      specId: { type: "string" },
    ...
    mutates: true,
    handler: (a, c) => ops.reviseSpec(c.universe.path, a as never),
  },
  {
    ...
    mutates: true,
    handler: (a, c) => ops.reviseOperation(c.universe.path, a as never),
  },
  {
    name: "remove_operation",
    description: "Pull an operation out of a DRAFT spec — the one you now believe is wrong, before a principal is asked to adopt it.\n\nThis is the verb whose absence made the asymmetry run backwards: `acknowledge_gap` lets you ADD a silencer constraining what a ratifier approves, and nothing let you REMOVE an operation you had come to believe was wrong.\n\nIt is a tombstone, not a delete: the operation stops applying, stops counting and stops rendering among the live ones, and stays readable as history with your reason on it. Refused once the spec is ratified; refused while somebody else's pending acknowledgement or comment hangs off it; and refused while another operation in the same draft names it as a criterion's target, which would leave that criterion with no rule.",
    inputSchema: obj({
      operationId: { type: "string" },
      reason: { type: "string", description: "Why. The ratifier is reading a proposal that changed shape." },
      model: { type: "string", description: "YOUR model id. Never guess it." },
      harness: { type: "string" },
    }, ["operationId", "reason"]),
    mutates: true,
    handler: (a, c) => ops.removeOperation(c.universe.path, a as never),
  },
  {
    name: "withdraw_spec",
    description: "Take a spec back. A DRAFT may always be withdrawn — nothing applied, and it is what ends a pre-approved gap's life along with the proposal it was attached to. An AGENT may withdraw its own principal's draft, and only that: nothing applied, so nothing is unbound, and taking back your own unratified proposal is authorship. It is refused while somebody else's pending gap or comment hangs off it, because withdrawing would leave their record with no proposal to be about.\n\nWithdrawing a RATIFIED spec TOMBSTONES what it introduced: its rules go to `retired`, which is the same end state a compensating `retire_requirement` reaches. Nothing is deleted, so nothing has to be counted first — an audit, pointer or population on one of those rules keeps resolving, and the rule simply stops binding (every queue filters on `ratified`). It is not a delete of the spec either: the row and its ratification stay, because removing a ratified act destroys the audit trail of the thing most worth auditing.\n\nThe one refusal left is the operation KIND. A spec that amended, retired or re-filed something that already existed cannot be withdrawn at all, however little touches it: putting the old statement back would need the witnesses taken when it was adopted, which the amendment re-baselined away, so the restored text would be witnessed against today's code as though the amendment never happened. That case is REPEAL — a new spec whose operations reverse this one's, which restores the text as its own witnessed act.",
    inputSchema: obj({
      specId: { type: "string" },
      reason: { type: "string", description: "Why. It stays on the record as the act it is." },
      model: { type: "string", description: "YOUR model id. Never guess it." },
      harness: { type: "string" },
    }, ["specId", "reason"]),
    mutates: true,
    handler: (a, c) => ops.withdrawSpec(c.universe.path, a as never),
  },
  {
