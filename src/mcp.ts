...
const tools: Tool[] = [
  ...
  {
    name: "withdraw_spec",
    description: "Take a spec back. A DRAFT may always be withdrawn — nothing applied, and it is what ends a pre-approved gap's life along with the proposal it was attached to. An AGENT may withdraw its own principal's draft, and only that: nothing applied, so nothing is unbound, and taking back your own unratified proposal is authorship. It is refused while somebody else's pending gap or comment hangs off it, because withdrawing would leave their record with no proposal to be about. A RATIFIED spec may be withdrawn only while its effects are still self-contained: nothing else operates on, audits, acknowledges, questions, asserts, points at or enumerates the rules it introduced. That is a reference count, so the store decides which backout applies, not the asker.\n\nIt is not a delete — the spec keeps its row and its ratification, because removing a ratified act destroys the audit trail of the thing most worth auditing. It is not a revert either: a spec that amended, retired or re-filed something that already existed is refused however little relies on it, since putting the old statement back would witness it against today's code as though the amendment never happened. That case is REPEAL — a new spec whose operations reverse this one's, which restores the text as its own witnessed act.",
    inputSchema: obj({
      specId: { type: "string" },
