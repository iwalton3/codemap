 ...
 * Which `git` to run.
 *
 * PATH is right nearly always, and is tried first on every platform. The
 * exception is Windows: Git for Windows installs to a fixed place but only puts
 * itself on the PATH of shells started AFTER installation, so a process launched
 * from an IDE, a service, or a stale session can have a PATH that has never heard
 * of it — and then every git call fails in a way that looks like "not a repo".
 *
 * Resolved lazily and once. `CODEMAP_GIT` overrides, for a git that is neither on
 * PATH nor where the installer puts it.
 */
let gitBinCache: string | undefined;
export function gitBin(): string {
  if (gitBinCache !== undefined) return gitBinCache;
  const override = process.env.CODEMAP_GIT?.trim();
  if (override) return (gitBinCache = override);
  if (spawnSync("git", ["--version"], { encoding: "utf8" }).status === 0) return (gitBinCache = "git");
  if (process.platform === "win32") {
    for (const c of [
      ...
      if (c && existsSync(c)) return (gitBinCache = c);
    }
  }
  // Nothing better to offer: fall back to the name so the failure is git's own
  // error rather than a path this invented.
  return (gitBinCache = "git");
}

function git(root: string, args: string[]): { ok: boolean; out: string } {
  const r = spawnSync(gitBin(), args, { cwd: root, encoding: "utf8" });
...
export function showFile(root: string, sha: string, path: string): Buffer | null {
  ...
  const r = spawnSync(gitBin(), ["show", `${sha}:${repoPrefix(root)}${path}`], { cwd: root, maxBuffer: 256 * 1024 * 1024 });
  if (r.status === 0) return r.stdout;
  // Not there as a blob. It may still be REACHABLE: a submodule is a gitlink (mode
  // 160000), not a tree, so the parent cannot see inside it and `git show` fails with
  // "exists on disk, but not in <sha>". Indexing already recurses through gitlinks
  // (`indexCommit`), so without this the anchors exist and their code does not —
  // a blank drill-down and an empty PR diff for exactly the code two products share.
  const link = gitlinkFor(root, sha, path);
  return link ? showFile(join(root, link.dir), link.oid, link.rest) : null;
}

/**
 * Gitlink cache, per (root, sha), for each directory probed. Holds negatives too —
 * the overwhelmingly common answer is "no submodule here", and it is the answer this
 * would otherwise pay a git process for on every miss.
 */
const gitlinkCache = new Map<string, Map<string, string | null>>();

/**
 * The submodule containing `path` at `sha`, if any, with the path inside it.
 *
 * Walks ancestors shortest-first, so the OUTERMOST gitlink wins — nesting is then
 * handled by `showFile` calling itself against the submodule's own repo, where the
 * same question is asked again with an empty repo prefix.
 */
function gitlinkFor(root: string, sha: string, path: string): { dir: string; oid: string; rest: string } | null {
  const key = `${root}\0${sha}`;
  let cache = gitlinkCache.get(key);
  if (!cache) { cache = new Map(); gitlinkCache.set(key, cache); }
  const prefix = repoPrefix(root);
  const parts = path.split("/");
  for (let i = 1; i < parts.length; i++) {
    const dir = parts.slice(0, i).join("/");
    let oid = cache.get(dir);
    if (oid === undefined) {
      const out = git(root, ["ls-tree", sha, `${prefix}${dir}`]).out;
      oid = /^160000 commit ([0-9a-f]+)\s/.exec(out.trim())?.[1] ?? null;
      cache.set(dir, oid);
    }
    if (oid) return { dir, oid, rest: parts.slice(i).join("/") };
  }
  return null;
}
...
export function lsTreeEntries(root: string, sha: string): TreeEntry[] | null {
  const r = spawnSync(gitBin(), ["-c", "core.quotePath=false", "ls-tree", "-r", "-l", "--full-name", sha], { cwd: root, encoding: "utf8", maxBuffer: 256 * 1024 * 1024 });
...
export function readBlobs(root: string, sha: string, paths: string[]): Map<string, string> {
  ...
  for (let i = 0; i < paths.length; i += CHUNK) {
    ...
    const r = spawnSync(gitBin(), ["cat-file", "--batch"], {
      cwd: root,
...
export function fetchRef(root: string, remote: string, refspec: string): { ok: boolean; err: string } {
  const r = spawnSync(gitBin(), ["fetch", "--no-tags", "--quiet", remote, refspec], { cwd: root, encoding: "utf8", timeout: 180_000 });
...
export function numstat(root: string, from: string, to: string): { path: string; adds: number; dels: number }[] | null {
  const r = spawnSync(gitBin(), ["-c", "core.quotePath=false", "diff", "--numstat", "-M", from, to], { cwd: root, encoding: "utf8", maxBuffer: 64 * 1024 * 1024 });
  if (r.status !== 0) return null;
...
export function diffHunks(root: string, from: string, to: string): Map<string, FileHunks> {
  const r = spawnSync(gitBin(), ["-c", "core.quotePath=false", "diff", "--unified=3", "--no-color", from, to], { cwd: root, encoding: "utf8", maxBuffer: 256 * 1024 * 1024 });
...
export function isAncestor(root: string, maybeAncestor: string, ref: string): boolean {
  return spawnSync(gitBin(), ["merge-base", "--is-ancestor", maybeAncestor, ref], { cwd: root }).status === 0;
}
...
export function changedFilesBetween(root: string, from: string, to: string): string[] {
  const r = spawnSync(gitBin(), ["-c", "core.quotePath=false", "diff", "--name-only", "-M", from, to], { cwd: root, encoding: "utf8", maxBuffer: 64 * 1024 * 1024 });
...
export function fetchAllPrHeads(root: string, remote = "origin"): { ok: boolean; err: string } {
  const r = spawnSync(gitBin(), ["fetch", "--no-tags", "--quiet", remote, `+refs/pull/*/head:refs/remotes/${remote}/pr/*`], {
...
export interface SubmoduleReport {
  drift: SubmoduleDrift[];
  /**
   * `git submodule status` itself failed in a repo that HAS git.
   *
   * Distinct from "no submodules", which succeeds with empty output. The
   * difference matters: a broken gitdir — a moved worktree, a half-finished
   * clone — makes the command fail, and the old code read that as "nothing to
   * report" and indexed on in silence. A scan that cannot tell whether its
   * submodules are in sync must say so, because the whole point of the guard is
   * that the index would otherwise describe code the commit does not ship.
   */
  error?: string;
}
 ...
 */
export function submoduleDrift(root: string): SubmoduleReport {
  // Not a git repo at all is not a submodule problem — a gitless universe is a
  // supported thing, and it has nothing to be out of sync with.
  if (!isGitRepo(root)) return { drift: [] };
  const r = spawnSync(gitBin(), ["submodule", "status"], { cwd: root, encoding: "utf8" });
  if (r.status !== 0) {
    return { drift: [], error: (r.stderr ?? "").trim().split("\n")[0] || `git submodule status exited ${r.status}` };
  }
