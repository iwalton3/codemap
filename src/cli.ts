...
/**
 * Move this store to a different sidecar, on purpose.
 *
 * Its own subcommand rather than a flag on `sync`, because it is a different act: sync
 * says "exchange with my team", this says "my team is now a different one". The counts
 * are the whole output — what is left behind is what somebody needs to have seen before
 * they type it, and it is why the refusal points here rather than just failing.
 */
async function cmdAdopt(root: string): Promise<void> {
  const r = await shared.adoptSidecar(root) as Record<string, any>;
  if (r.error) { console.error(r.error); process.exit(1); }
  console.log(`sidecar ${r.sidecar}  (${r.universe})`);
  console.log(`  adopted ${String(r.lineage).slice(0, 12)}${r.was ? `, was ${String(r.was).slice(0, 12)} at ${r.wasAt}` : ""}`);
  for (const sc of r.replaced as string[]) console.log(`  REPLACED on the next read: rows folded from ${sc}`);
  console.log(`  ${r.note}`);
}
...
async function cmdInit(root: string): Promise<void> {
  ...
  console.log(`baseline commit: ${r.commit ?? "(no git)"}${r.commit && !(r as { snapshotSkipped?: string }).snapshotSkipped ? ` (snapshotted, branch ${r.branch ?? "?"})` : ""}`);
  if ((r as { snapshotSkipped?: string }).snapshotSkipped) {
    console.log("note: the working tree has uncommitted changes, so the commit's snapshot was not taken from it; "
      + "`codemap snapshot` caches it from git objects.");
  }
...
/** `--ref` forces a rebuild, and it is the repair `snapshotRefusal` names. */
...
if (positionals[0] === "analyze") {
  ...
  if (positionals[0] === "pr-packet") {
    ...
    await cmdAdopt(resolve((values.repo as string | undefined) ?? positionals[2] ?? "."));
